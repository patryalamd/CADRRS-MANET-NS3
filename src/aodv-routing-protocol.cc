...
else
{
    ...
    double reliabilityScore =
        (0.5 * linkStability) +
        (0.3 * (1.0 - congestionLevel)) +
        (0.2 * (1.0 / static_cast<double>(hop)));

    newEntry.SetReliabilityScore(
        reliabilityScore);
   ...
   NS_LOG_UNCOND(
    ...
    if (m_routingTable.LookupRoute(dst, toDst))
    {
        ...
        if (
            ...
            (
    rrepHeader.GetDstSeqno() == toDst.GetSeqNo()
    &&
    (
        newEntry.GetReliabilityScore() >
        toDst.GetReliabilityScore()
        ||
        hop < toDst.GetHop()
    )
))
